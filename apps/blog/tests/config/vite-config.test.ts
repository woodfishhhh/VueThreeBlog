...
describe("Vite+ task graph", () => {
  it("keeps agent-oriented tasks while remaining workspace-compatible", () => {
    ...
    expect(config.run?.cache).toBeUndefined();
    ...
    expect(config.run?.tasks?.["agent:dist"]?.command).toContain(
      "tsx ../../packages/content-tools/src/cli.ts verify-dist --app-root .",
    );
    expect(config.run?.tasks?.["agent:dist"]?.command).not.toContain("vp run dist:verify");
    expect(config.run?.tasks?.["deploy:build"]?.command).toContain(
      "tsx ../../packages/content-tools/src/cli.ts verify-dist --app-root .",
    );
