...
<template>
  <nav
    class="site-nav-gradient pointer-events-none fixed left-0 top-0 z-50 flex w-full items-center justify-between p-6"
  >
    <RouterLink
      :to="getRouteLocationForSiteMode('home')"
      class="pointer-events-auto cursor-pointer text-xl font-bold tracking-widest text-[var(--stage-fg)] mix-blend-difference"
      @click="handleNav()"
    >
      WOODFISH
    </RouterLink>
    ...
    <div class="pointer-events-auto hidden items-center gap-3 md:flex">
      <div data-nav-group="desktop" class="flex gap-8">
        <RouterLink
          ...
          :class="isActive(item.id) ? 'text-[var(--accent)]' : 'text-[var(--stage-hint)] hover:text-[var(--stage-fg)]'"
      ...
      <ThemeToggle data-nav-theme-toggle="desktop" :theme="theme" @toggle-theme="handleToggleTheme" />
    </div>

    <div class="pointer-events-auto flex items-center gap-2 md:hidden">
      <ThemeToggle data-nav-theme-toggle="mobile" :theme="theme" @toggle-theme="handleToggleTheme" />
      <button
    ...
    <Transition name="fade-slide">
      <div
        v-if="isOpen"
        class="pointer-events-auto absolute right-6 top-20 flex min-w-[150px] flex-col gap-4 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-2)] p-4 shadow-lg backdrop-blur-md md:hidden"
      >
        <RouterLink
          v-for="item in navItems"
          :key="item.id"
