...
const hasBlogReturnContext = computed(
  () => Object.keys(blogReturnQuery.value).length > 0,
);
...
const backLinkLabel = computed(() =>
  hasBlogReturnContext.value ? "Back to Blog" : "Back Home",
);

let requestToken = 0;

// slug 校验：先 sanitize 再用，防止超长或含控制字符的 slug 引发问题
const incomingSlug = computed(() =>
  sanitizeSlug(String(route.params.slug ?? "")),
);
...
watch(
  ...
  async (slug) => {
    ...
    if (canonicalSlug) {
      const currentIndex = postSummaries.findIndex(
        (post) => post.canonicalSlug === canonicalSlug,
      );
      previousPost.value =
        currentIndex > 0 ? (postSummaries[currentIndex - 1] ?? null) : null;
      nextPost.value =
        currentIndex >= 0 ? (postSummaries[currentIndex + 1] ?? null) : null;
    }
  ...
  <main ref="pageRoot" class="article-page">
    ...
    <div class="article-page__nav-shell">
      ...
      <div class="article-page__nav-actions">
        <span v-if="resolvedSlug" class="article-page__nav-meta"
          >/ posts / {{ resolvedSlug }}</span
        >
    ...
    <div
      v-if="isLoading"
      class="article-page__status"
      data-testid="post-view-loading">
    ...
    <div
      ...
      <p class="article-page__status-text">
        The requested route does not map to an available post in this generated
        archive.
      </p>
    </div>
    ...
    <div
      ...
      <nav
        ...
        <RouterLink
          ...
          <strong class="article-page__adjacent-title">{{
            previousPost.title
          }}</strong>
          <span class="article-page__adjacent-meta">{{
            previousPost.publishedLabel
          }}</span>
        </RouterLink>
